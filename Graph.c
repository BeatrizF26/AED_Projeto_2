...
Graph* GraphCreateTranspose(const Graph* g) {
    ...
    //Inicia-se a criação de um grafo transposto com o mesmo número de vértices do grafo passado como argumento
    Graph* transpose = GraphCreate(g->numVertices, g->isDigraph, g->isWeighted);

    //Percorre todos os vértices do grafo original
    ListMoveToHead(g->verticesList);
    while (ListCurrentIsInside(g->verticesList)) {
        //Por cada vértice, guarda as arestas que esse vértice tem
        struct _Vertex* currentVertex = ListGetCurrentItem(g->verticesList);
        List* edges = currentVertex->edgesList;

        //Adiciona as novas arestas (transpostas) ao grafo transposto
        ListMoveToHead(edges);
        while (ListCurrentIsInside(edges)) {
            //Cada aresta tem a direção invertida e é adicionada ao novo grafo
            struct _Edge* edge = ListGetCurrentItem(edges);
            GraphAddEdge(transpose, edge->adjVertex, currentVertex->id);
            ListMoveToNext(edges);
        }
        //Quando se verificar todas as aresta de um vértice,
        //Passa-se para o próximo vértice e repete-se todo o processo
        ListMoveToNext(g->verticesList);
    }

    //No fim retorna-se o grafo transposto criado
    return transpose;
}
